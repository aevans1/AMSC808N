...
% [w,f,gnorm] = SGD(fun, gfun, Hvec, Xtrain,label,w, bsz, kmax, tol);
% method = 'SGD';
...
% tol = 1e-4;
% 
% fun = @(I,w)lsq_loss(I,Xtrain,label,w);
% w = ones(dim^2+dim+1,1);
% 
% [w,f,gnorm] = GaussNewton(fun,Xtrain,label,w,kmax,tol);
% method = 'GaussNewton';
% 
% % [w,f,gnorm] = LevenburgMarquardt(fun,Xtrain,label,w,kmax,tol);
% % method = 'LevenMarq'
%%
...
fname = sprintf('fval_%s_dim%d.png', method, dim);
saveas(fig, fname);
...
set(gca,'fontsize',fsz,'Yscale','log');
fname = sprintf('gnorm_%s_dim%d.png', method, dim);
saveas(fig, fname);
